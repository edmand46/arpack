...
`},
	...
	if schema.Messages[0].PackageName != "framework" || schema.Messages[1].PackageName != "game" {
		t.Fatalf("expected per-message package names preserved, got %+v", schema.Messages)
	}
}
